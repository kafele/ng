...
class Tolk extends Base_Controller 
{
	var $mimes			= array();
	...
	public function __construct()
	{
		parent::__construct();
	 echo('tolkuchka');	
		//$this->load->model('Tolk_model', 'tolk');
...
//$data= array(
//'ad_title' => "secondTitle",
//'ad_message' => "second message"
//);
...
$this->db->where('tolk_message_id' , $messageID);
$this->db->delete('tolk');
...
echo "Удалено";
	...
	{
		// Upload folder
		$upload_path = config_item('tolk_folder');
		...
		if ($user && $upload_path != false && $upload_path !='')
		{
			...
			$fancygroup = Connect()->get_group(config_item('tolk_group'));
			...
			if ($user['email'] == $email && $usergroup['level'] >= $fancygroup['level'])
			{
				...
				{
					$this->error(lang('module_tolk_invalid_upload'));
				}
				else
				{
					...
					if (config_item('tolk_file_prefix') == '1')
					{
						$new_file_name = $email . '_' . $new_file_name;
					}				
				
					if ( ! @move_uploaded_file($_FILES['Filedata']['tmp_name'], config_item('tolk_folder') . $new_file_name))
					{
						$return['status'] = '0';
					}
					else
					{
 						...
 						if (config_item('tolk_send_alert') == '1' && config_item('tolk_email') != '')
 						{
 							$to = config_item('tolk_email') ;
							
							$subject_admin = lang('tolk_alert_mail_subject') . ' : ' . $user['screen_name'];
							...
							$data = array(
								...
								'upload_folder' =>	config_item('tolk_folder')
							);
							
							// Email to Admin
							$message = $this->load->view('emails/tolk_upload_admin_alert', $data, true);
							
							$this->send_mail($to, $message, $subject_admin);
 						}
					}
					$return['src'] = config_item('tolk_folder') . $new_file_name;
			...
			{
				$this->error(lang('module_tolk_no_right'));
