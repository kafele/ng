...
return  $c;	
	...
	public function build($full=true ) // full -значит не убирать limit 
	{
 $p=$this->searchparams;
$CI =& get_instance();		 
...
$this->limit = (config_item('realt_ads_per_page')>0)?config_item('realt_ads_per_page'): 25;   //  выводить на страницу
if ($this->table == "sutki") {
$this->limit = (config_item('realt_ads_per_page_sutki') > 0) ? config_item('realt_ads_per_page_sutki') : $this->limit; //  выводить на страницу сутки
}
...
parse_str($_SERVER['QUERY_STRING'], $_GET);
 ...
 //////// Дополнительно смотрим категорию , если это в URL то добавляем в get
 $CI =& get_instance();
...
if ($URIcat_id){  // Если в url есть категория
$this->searchparams['ad_catid']=$URIcat_id;
 // Если   категория = 11 то это квартиры на сутки
 // тогда меняем итаблицу
