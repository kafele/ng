...
if (!$CI){$CI =& get_instance();$CI->load->library('parser');}
	...
	if  ($this->data['mlev']!=4 && $CI->uri->segment(4)!="label"){
	return;
	}
...
if ($uid=="") {	echo "не задан uid ";	return;	}
  break;
  
case 'label':
$showtype="label";
$label=$CI->uri->segment(5);
if ($label=="") {	echo "не задан label ";	return;	}
/// Получем список объявлений с данным лэйбом
$AdsWithLabelArr  = getAdsWithLabel($label);
if (count($AdsWithLabelArr)==0	) {	

$this->data['realt']="Нет объявлений с такой меткой.";
...
if ($showtype=="label"){
$CI->db->where_in('ad_id', $AdsWithLabelArr );
}
...
";
	  ...
	  $complaints=getUIDComplaints($uid); 
	$str_add .=$complaints;
