...
$str_add .="<div><h1>«Умный поиск»  тестируется.</h1> Пишите замечания на info@neagent.by </div>";
...
if ($priceMax==0){$priceMaxForm="";}else{$priceMaxForm=$priceMax;};
 ...
 $this->data['searchform']= $CI->parser->parse('searchform_input', $addata);

 
 $this->data['search_filter']= $CI->parser->parse('searchfilter', $addata);
 $str_add .=$this->data['search_filter'];
...
$string_ad = read_file($this->config->item('module_path').'Realt/views/'.$this->data['view_prefix'].'realt_ad.php');
...
if ($row->ad_area == '0' && $row->ad_subarea == 'NULL'){
$str_add .= "!!Район не указан";
}
...
$str_add .= $CI->parser->parse($this->data['view_prefix'] . 'realt_ad', $addata);
