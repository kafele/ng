...
class Ads_list   extends Base_model
{
    ...
    $search_qs1= $this->addorder ($param .'_az');
...
$ord = explode("_", $getorder); 
 if ( $ord[0] ==$param) {
$classstring=" class='currorder'";

if ( $ord[1] =='az'){
$class1=" class='currorder'";
}
else{
$class2=" class='currorder'";
}
 ...
 // по умолчанию
	...
	$str=" <span". $classstring. ">" . $title ."</span><a href='" . $url_part . "?".  $search_qs1 ."' title='в порядке увеличения' ".$class1.">▲</a><a href='" . $url_part . "?". $search_qs2 ."' title='в порядке уменьшения' ".$class2.">▼</a>";
	//$str=$param ;
	return $str;
...
$config['base_url']=$CI->realt_route_model->pagination_base_url; // http://neagent.by/page/﻿
$config['uri_segment']=$CI->realt_route_model->page_segment;
echo("nnn".$config['base_url']);
...
if ($params['cidbcriteria']->table == "sutki") {
...
$config['per_page'] =$params['cidbcriteria']->limit;
...
echo("nnn1".$config['base_url']);

if ($_GET['mode'] == 'search') {
$config['base_url'] = "http://neagent.by/board/?".$search_qs ;
// пока удалено если wap моде$config['base_url'] = "http://neagent.by/wap/?".$search_qs ;
$config['page_query_string'] = TRUE;
//echo("++mode+search++".$config['base_url']);
}
else{
//echo("888888888888888888888");
$s_qs=(string)$_SERVER['QUERY_STRING'];
//суффиксом может быть только сортировка? если это не поиск -НЕТ 
//  все может быть суффиксом
$config['suffix'] ='?' .$s_qs;
...
echo("nnn2".$config['base_url']);


$config['full_tag_open'] ='<div class="pagination" style="height:50px;"><div class="page_numbers"><span class="page-first">'.lang('module_realt_pagination_page').'</span>';
...
$datesortlink=$this->createSortLink("postdate" , "по дате");
$pricesortlink=$this->createSortLink("defprice" , "по цене");
...
$str_add .= "<div> <div  style='width:55%;float:left;padding:10px;'> <div style='width:100%;background:white;padding:10px;padding-left:0px;padding-bottom:3px;font-size:18px; '> Объявления от собственников. </div><div style='padding-left:0px;padding-bottom:10px;font-size:14px;'>Нажимайте 'сообщить о нарушении', если увидите агента.</div> ";
...
$str_add .= " </div> <div  style='width:40%;float:left;padding:10px;'>";
...
{
array_push ($maparr, $ads[$i]->fields['ad_id']);
}
if  (count($maparr)==0){
$maparr=array(0,0);
}
...
print_r($maparr);
 ...
 //if ($this->data['showmap'] && $mcat == 11) {
            ...
            $mparams = array(
                'ads' => $maparr,
				'table'=> $params['cidbcriteria']->table,
            );
			$str_map_params  = get_map_params2($mparams); //получает параметры для передачи в карту
            $str_map_code  = get_map_code(); //получает код вставки карты
...
$str_add .="<div style='width:100%;    '>" . $str_map_params . $str_map_code  .   "</div>";

$str_add .="<div style='width:100%;height:100px; margin-bottom:10px;background:#3195df;'><div style='padding:20px;'><a href='http://neagent.by/n' TARGET = '_blank' style='color:white; font-size:24px;line-height:28px;'>Проверить номер телефона  на наличие в базе агентов</a></div> </div>";
//$str_add .="<div style='width:100%;height:100px;border:1px solid grey;margin-bottom:10px;'>Есть предложения от посредников по вашему запросу<br>Заказать звонок</div>";
