...
//print_r ($subareasArray);
$subarr_values=array();
if ($_GET['subar']){
foreach ($subareasArray as $i => $value) {
if ($value){
array_push($subarr_values,$value);
}
}
}
...
// Перевод цены в универсальную  с долларов , т.к. поиск в долларах
...
if ($addata['ad_showpolitic']=="true"||$addata['ad_showpolitic']=="combi"||$addata['ad_showpolitic']=="true2"){
  if ($this->data['userstatus']!="active"){
   $addata['ad_phones']="Автор скрыл свой номер. Его видят не все посетители сайта.";
   }
