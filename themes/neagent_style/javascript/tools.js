...
$(document).ready(function() {
 ...
 if ($('#f_ads')){
   ...
   var search_bar = $('#f_ads');
    if (search_bar.offset()){
   
   
   var margin = 10;
   var borderY = search_bar.offset().top - margin;
   var leftX = search_bar.offset().left  ;
   // аОаБбаАаБаОббаИаК аБбаДаЕб аВбаЗбаВаАбб аПаОббаОбаНаНаО аВ аПбаОбаЕббаЕ аПбаОаКбббаКаИ аДаОаКбаМаЕаНбаА
   $(window).bind('scroll', function()
   {
   //alert($('.footTop').position().top);
   if ($('.footTop').offset().top<$(window).height()){
   fff=1;
   }
   else
   {
   ...
   if ($(window).height() > $('#f_ads').height() && fff==0){
   return;
  // alert ($(window).height());
  // alert ($('.ib-ins').height());
     // аЗаАаПаОаМаИаНаАаЕаМ аНаА баКаОаЛбаКаО аПаИаКбаЕаЛаЕаЙ баЕаЙбаАб аПбаОаКбббаЕаН аДаОаКбаМаЕаНб
     var currentY = $(document).scrollTop();
     if(currentY > borderY)
     {
       // аПаОбаА ТЋаВбббаВаАббТЛ баЛаЕаМаЕаНб аИаЗ аДаОаКбаМаЕаНбаА аИ аПбаИаЛаЕаПаЛббб аЕаГаО аК аОаКаНб;
       // аДаЛб ббаОаГаО аПбаИбаВаАаИаВаАаЕаМ аЕаГаО баВаОаЙббаВаАаМ CSS
       search_bar.css({position: 'fixed', top: margin + 'px'  });
	   
     } 
     else if(currentY < borderY)
     {
       // баЕаПаЕбб аПаОаЛбаЗаОаВаАбаЕаЛб аПбаОаКбббаИаЛ аОаКаНаО аОаБбаАбаНаО аВаВаЕбб,
       // аПаОбаА аВаЕбаНббб баЛаЕаМаЕаНб аНаА аИббаОаДаНаОаЕ аМаЕббаО;
       // аДаЛб ббаОаГаО аВаОаЗаВбаАбаАаЕаМ аИаЗаМаЕаНбаНаНбаЕ аЗаНаАбаЕаНаИб баВаОаЙббаВ CSS
       // аК аИаЗаНаАбаАаЛбаНбаМ
       search_bar.css({position: '', top: ''});
	   //leftX = search_bar.offset().left  ;
     }
