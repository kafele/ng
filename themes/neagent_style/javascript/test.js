...
var FacebookList2 = new Class({
  ...
  autoHideInput: function() { 
     ...
     newId=this.autoholder.id 
	  newId=newId.replace('-auto','-demoinput')
  ...
  autoFocus: function(el) {
	...
	//alert( this.autocurrent  )
	
     if(this.autocurrent) {
	 try{
	 this.autocurrent.removeClass('auto-focus');
	 }
	 catch(err)
  {}
...
window.addEvent('domready', function() {
  ...
  // init
  var tlist2 = new FacebookList2('ul-demo', 'ul-auto');
  
  // fetch and feed
  new Request.JSON({'url': 'http://neagent.by/realt/streets', 'onComplete': function(j) {
