...
divIndex=divIndex+1; 
  $('<div id="progress' +divIndex +'" style="display: none;"><table cellpadding="0" cellspacing="0"><tr><td> <img alt="Идёт загрузка...." src="http://tolkuchka.by/themes/tolk/assets/images/ajax-loader.gif" /> Загрузка </td></tr></table> </div>').insertAfter('#progress');

    form = typeof(form)== "string"?$('#'+form):form;
    var iframe = document.createElement("iframe");
    iframe.setAttribute("id","ajax-temp"+divIndex);
    iframe.setAttribute("name","ajax-temp"+divIndex);
    iframe.setAttribute("width","0");
    iframe.setAttribute("height","0");
    iframe.setAttribute("border","0");
    iframe.setAttribute("style","width: 0; height: 0; border: none;");
    form.parentNode.appendChild(iframe);        
    window.frames['ajax-temp'+divIndex].name="ajax-temp"+divIndex;        
        if (id_element != 'logo'){
            $('#progress'+divIndex).show();
			 //alert ("показан прог");
            $('#files').show();
			//alert ("показаны файлы");
        }
		
        $('#ajax-temp'+divIndex).load(function(){
            doUpload();
        });        
    form.setAttribute("target","ajax-temp"+divIndex);
...
function doUpload(){
    var frame = $('#ajax-temp'+divIndex).contents().find('body');
    var imageName = frame.html();
    frame.empty();    
    $('#progress'+divIndex).hide(); 
...
alert(imageName);
    ...
    $('#ajax-temp'+divIndex).unload(function(){            
        doUpload();
    });
}